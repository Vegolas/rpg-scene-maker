...
// vocabulary. System is the active game system's id (issue #127; null = none chosen), already normalized
// through GameSystemRegistry. The panel mirrors this exact shape by hand in
// AmbientDirector.Ui/Contracts/PartyContracts.cs — keep the two in sync when a field changes.
public record PartyDto(List<PartyMember> Players, List<PartyCounter> Counters, List<Enemy> Enemies, string? System);
