...
// Mirrors the API's PartyMember/PartyCounter shapes (Models/PartyMember.cs) — contracts are duplicated per
// project by design, so keep the two in sync by hand when a field changes. The wire vocabulary is deliberately
// "players" (GET /party/list returns players; ids appear in /party/players/{id} urls) even though the API's C#
// type is PartyMember (renamed there to avoid colliding with the audio "player"). A counter is a generic
// {label, value, max, style}: style is null | "pips" | "number" — null lets the renderer decide (dots when a
// max is set, else a bare number); "pips" needs a small max (1–24). Table-level counters (e.g. Fear) live
// alongside the players in the same envelope. The Daggerheart/Fear presets are a UI-only convenience — the API
// stores only generic counters.
public record PartyDto(List<PartyPlayerDto> Players, List<PartyCounterDto> Counters);
